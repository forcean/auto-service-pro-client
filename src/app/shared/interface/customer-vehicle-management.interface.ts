...
export type DiagnosticStatus = 'OVERDUE' | 'REPLACE' | 'DUE_SOON' | 'FAIR' | 'GOOD';
...
export interface OverviewActiveWorkOrder {
  ...
  title: string;            // เช่น "Brake Inspection"
...
export interface OverviewActivityLog {
  id: string;
  date: string;
  title: string;            // เช่น "เปลี่ยนน้ำมันเครื่อง"
  byUser?: string;
}
...
export interface IVehicleOverviewState {
  ...
  internalNote?: string;    // บันทึกช่าง
}